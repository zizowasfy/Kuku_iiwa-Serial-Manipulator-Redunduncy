...
jointsRange = [pi/6 pi/3 ; -pi/2 pi/2 ; -pi/2 pi/2 ; -pi/2 pi/2]; %%%%%%%%%%%%%%
%% PsuedoInverse
...
q = [deg2rad(5), deg2rad(30), deg2rad(60), deg2rad(90)]';          % Initial Position in Joint Space
FK = localFK(q,L);
...
while (sum(abs(final - current)) > 0.001)
    ...
    dq0 = calc_dQ0([1,1,1,1], q, "range", jointsRange);     %%%%%%%%%
    J = [Jacobian(1:2,:) ; Jacobian(6,:)] ;     %%%%%%%%%
    Joint_step = J_hash * Task_step' + (eye(4) - J_hash*J)*dq0;
